...
function runBatchFA2(graph, settings, doneIterations, finalCallback) {
  ...
  console.log(
    " FA2 batch of " + batchIterations + " iterations processed in:",
    (Date.now() - t0) / 1000 + "s",
  );
  doneIterations += batchIterations;
  if (doneIterations < FA2Iterations) runBatchFA2(graph, settings, doneIterations, finalCallback);
...
function processGraph(graph) {
  ...
  graph.forEachEdge((edge, { weight }, n1, n2, n1_attrs, n2_attrs) => {
    graph.setEdgeAttribute(
      ...
      Math.max(
        ...
        Math.log((total * weight) / (n1_attrs[links_type] * n2_attrs[links_type])),
      ),
    );
  });

  // Run Louvain to field community
  if (entity === "characters") louvain.assign(graph, { resolution: 1.2 });

  // Spatializing with FA2
  console.log(
    "Starting ForceAtlas2 for " + FA2Iterations + " iterations by batches of " + batchIterations,
  );
  ...
  runBatchFA2(graph, settings, 0, function (doneIterations) {
    ...
    console.log(
      "ForceAtlas2 fully processed in:",
      (time1 - time0) / 1000 + "s (" + doneIterations + " iterations)",
    );
